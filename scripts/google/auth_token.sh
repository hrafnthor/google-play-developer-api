...
print_usage () {
  ...
  -k  GOOGLE_PLAY_API_CLIENT_PRIVATE_KEY
...
while getopts 's:e:k:d:' flag; do
  case "${flag}" in
    ...
    e) GOOGLE_PLAY_API_CLIENT_EMAIL="${OPTARG}" ;;
    k) GOOGLE_PLAY_API_CLIENT_PRIVATE_KEY="${OPTARG}" ;;
    d) GOOGLE_PLAY_API_CLIENT_AUTH_TOKEN_EXPIRATION_SECONDS=${{OPTARG}} ;;
    *) print_usage
       exit 1 ;;
  esac
done
...
if [ -z ${GOOGLE_PLAY_API_CLIENT_EMAIL+x} ]; then
    error "Missing required 'GOOGLE_PLAY_API_CLIENT_EMAIL' input. Pass it directly via '-e' flag or set as env var"
    exit 1
fi
if [ -z ${GOOGLE_PLAY_API_CLIENT_PRIVATE_KEY+x} ]; then
    error "Missing required 'GOOGLE_PLAY_API_CLIENT_PRIVATE_KEY' input. Pass it directly via '-k' flag or set as env var"
    exit 1
fi
if [ -z ${GOOGLE_PLAY_API_CLIENT_AUTH_TOKEN_EXPIRATION_SECONDS+x} ]; then
    GOOGLE_PLAY_API_CLIENT_AUTH_TOKEN_EXPIRATION_SECONDS=180
fi

JWT_HEADER=$(echo -n '{"alg":"RS256","typ":"JWT"}' | openssl base64 -e)
JWT_ISSUED_AT_DATE=$(date +%s)
JWT_EXPIRATION_DATE=$(($(date +%s)+$GOOGLE_PLAY_API_CLIENT_AUTH_TOKEN_EXPIRATION_SECONDS))
JWT_BODY=$(jq  --null-input \
    --arg aud "${GOOGLE_PLAY_API_TOKEN_URI}" \
    --arg iss "${GOOGLE_PLAY_API_CLIENT_EMAIL}" \
...
JWT_SIGNATURE=$(echo -n "$JWT_PAYLOAD" | openssl dgst -binary -sha256 -sign <(printf '%s' "$GOOGLE_PLAY_API_CLIENT_PRIVATE_KEY") | openssl base64 -e)
