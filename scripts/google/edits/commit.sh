...
HTTP_RESPONSE=$(curl --write-out "HTTPSTATUS:%{http_code}" \
    ...
    --silent \
    --request POST \
    https://androidpublisher.googleapis.com/androidpublisher/v3/applications/${GOOGLE_PLAY_API_PACKAGE_NAME}/edits/${GOOGLE_PLAY_API_EDIT_ID}:commit)
...
if [[ ${HTTP_STATUS} != 200 ]]; then
    ...
    info "Body: $HTTP_BODY"
    error "Failed to commit edit ${GOOGLE_PLAY_API_EDIT_ID}. Exiting."
    exit 1
fi
